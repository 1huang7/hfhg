...
class QiannanTourismApp {
    ...
    handleAttractionHover(event) {
        ...
        if (badge) {
            badge.style.boxShadow = '0 0 20px rgba(39, 174, 96, 0.6)';
        }
        card.style.boxShadow = '0 15px 40px rgba(39, 174, 96, 0.2)';
    }
    ...
    handleRouteHover(event) {
        ...
        item.style.boxShadow = '0 5px 20px rgba(39, 174, 96, 0.2)';
    }
    ...
    // 切换斗篷山详细介绍
    toggleDoupengDetails() {
        const detail = document.getElementById('doupeng-detail');
        const btn = document.querySelector('[onclick="toggleDoupengDetails()"]');
    ...
    // 切换贵州西南大草原详细介绍
    toggleGrasslandDetails() {
        const detail = document.getElementById('grassland-detail');
        const btn = document.querySelector('[onclick="toggleGrasslandDetails()"]');
    ...
    // 切换中国天眼景区详细介绍
    toggleTianyanDetails() {
        const detail = document.getElementById('tianyan-detail');
        const btn = document.querySelector('[onclick="toggleTianyanDetails()"]');
    ...
    // 切换都匀石板街详细介绍
    toggleShibanDetails() {
        const detail = document.getElementById('shiban-detail');
        const btn = document.querySelector('[onclick="toggleShibanDetails()"]');
...
function toggleDoupengDetails() {
    if (window.qiannanApp) {
        window.qiannanApp.toggleDoupengDetails();
    }
}

function toggleGrasslandDetails() {
    if (window.qiannanApp) {
        window.qiannanApp.toggleGrasslandDetails();
    }
}

function toggleTianyanDetails() {
    if (window.qiannanApp) {
        window.qiannanApp.toggleTianyanDetails();
    }
}

function toggleShibanDetails() {
    if (window.qiannanApp) {
        window.qiannanApp.toggleShibanDetails();
    }
}

// 页面加载完成后初始化
document.addEventListener('DOMContentLoaded', () => {
    window.qiannanApp = new QiannanTourismApp();
    ...
    主要景点：
    - 斗篷山：原始森林覆盖，云雾缭绕
    - 乌蒙大草原：贵州屋脊，草原佛光、天池、杜鹃花海
    - 中国天眼：世界最大射电望远镜，探索宇宙奥秘
    - 都匀石板街：明清历史文化街区，青石板路，民族风情
